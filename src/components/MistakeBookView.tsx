...
export default function MistakeBookView({ onSelectQuestion }: { onSelectQuestion: (id: string) => void }) {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      <h1 className="text-xl font-bold text-gray-700 mb-4">📋 错题本</h1>
      {/* Subject Tabs */}
      <div className="flex gap-2 mb-3">
        {(['all', 'geography', 'biology'] as const).map(s => (
          <button key={s} onClick={() => setSubjectFilter(s)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium ${subjectFilter === s ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab('reviewing')} className={`px-4 py-1.5 rounded-full text-sm font-medium ${tab === 'reviewing' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500'}`}>攻克中</button>
        <button onClick={() => setTab('mastered')} className={`px-4 py-1.5 rounded-full text-sm font-medium ${tab === 'mastered' ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-500'}`}>已掌握</button>
      </div>
      {/* List */}
          ...
          {filtered.map((m: any) => m.question && (
            <div key={m.question_id} onClick={() => onSelectQuestion(m.question_id)}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-700 truncate">{m.question.chapter || '未分类'}</div>
                <div className="text-xs text-gray-400">复习 {m.review_count} 次 · {m.is_mastered ? '✅ 已掌握' : '🔄 复习中'}</div>
              </div>
            </div>
          ))}
        </div>
