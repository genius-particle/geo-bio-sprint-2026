...
export default function DashboardView({ onCapture, onSelectQuestion, refreshKey }: {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      <h1 className="text-2xl font-bold text-green-600 mb-1">🌍🔬 地生冲刺</h1>
      <p className="text-gray-400 text-sm mb-4">八年级地生会考备考工具</p>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-white rounded-xl p-3 text-center shadow-sm">
          <div className="text-2xl font-bold text-blue-500">{questions.length}</div>
          <div className="text-xs text-gray-400">总录入</div>
        </div>
        <div className="bg-white rounded-xl p-3 text-center shadow-sm">
        ...
        <div className="bg-white rounded-xl p-3 text-center shadow-sm">
          ...
          <div className="text-xs text-gray-400">错题数</div>
        </div>
      </div>
      ...
      {/* Recent Questions */}
      <h2 className="text-sm font-semibold text-gray-500 mb-2">最近录入</h2>
        ...
        {recent.map(q => (
          <div key={q.id} onClick={() => onSelectQuestion(q.id)}
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium text-gray-700 truncate">{q.chapter || '未分类'}</div>
              <div className="text-xs text-gray-400">{SUBJECT_LABELS[q.subject]} · {q.analysis_status === 'pending' ? '⏳ 待分析' : '✅ 已分析'}</div>
            </div>
          </div>
        ))}
      </div>
