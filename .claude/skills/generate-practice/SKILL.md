...
4. 如果用户说「快问快答」或「生成今天的练习」→ 扫描该学科**全部** wiki 知识点（等同"给所有 X 知识点出题"）
5. 如果用户未指定 → 列出所有可用知识点让用户选择
...
**快问快答模式下，每个知识点生成恰好 1 道题，必须覆盖全部知识点。** 不允许遗漏知识点，不允许一个知识点出多道题。

#### 出题数量

- **快问快答**：每个知识点 **1 道题**，覆盖 `data/wiki/` 中该学科全部页面
- **指定知识点练习**：每个知识点 5-8 道题目（仅当用户明确指定少数知识点时使用）
...
8. **中文引号**：题目文本中的中文引用（如「绿水」「二调」）禁止使用 ASCII 双引号 `"`，必须使用中文引号 `""`；ASCII `"` 只用于 JSON 结构

## 质量门禁（写入文件后必须执行）

### JSON 有效性 + 中文引号校验

写入 JSON 文件后，**必须立即**用 Python 验证 JSON 可解析且无未转义双引号：

```python
import json
with open('文件路径') as f:
    data = json.load(f)  # 如果失败说明有未转义双引号
```
